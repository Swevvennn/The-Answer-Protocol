...
// Les deux "écrans" possibles de l'application
...
struct MyApp {
    ...
    // champ utilisé pendant la saisie, avant validation
    username_input: String,
    // username validé, une fois qu'on est passé sur ShowData
    username: String,
 
    // ton contenu existant, maintenant affiché uniquement une fois le username validé
    counter: i32,
	text: String,
	val: f32,
	check: bool,
    ...
    fn show_username_screen(&mut self, ui: &mut egui::Ui) {
        ui.vertical_centered(|ui| {
            ...
            if (enter_pressed || button_clicked) && !self.username_input.trim().is_empty() {
                ...
                // ici, une fois le username validé, tu peux par exemple
                // charger ton fichier json / tes données pour cet utilisateur
                // let contents = fs::read_to_string("exemple.json")
                //     .expect("Should have been able to read the file");
